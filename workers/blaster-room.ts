 ...
 *
 * The Bug Blaster twin of GalaxySwarmRoom, and deliberately identical in
 * shape: this DO holds no game state at all. The host browser is the
 * single authoritative simulation; the room only handles seating
 * (host = seat 0) and relays frames — guest inputs up to the host, host
 * snapshots down to the guest. Snapshots are transient and never touch
 * storage, so an active game costs the DO almost nothing.
 */
export class BugBlasterRoom extends DurableObject<Env> {
	...
	private stateFor(pid: string): BlasterRoomState {
	...
	private broadcastRoom() {
		this.hub.broadcastEach((att) => ({ t: "room", state: this.stateFor(att.pid) } satisfies BlasterServerMsg));
	}
	...
	async fetch(request: Request): Promise<Response> {
		...
		if (seat === -1) {
			if (this.room.seats.length >= BLASTER_MAX_PLAYERS) {
				const pair = new WebSocketPair();
				pair[1].accept();
				pair[1].send(JSON.stringify({ t: "error", message: "This room already has two players." } satisfies BlasterServerMsg));
	...
	async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer) {
		...
		let msg: BlasterClientMsg;
		try {
			msg = JSON.parse(message) as BlasterClientMsg;
		} catch {
			return;
		}
		...
		switch (msg.t) {
			case "start": {
				...
				if (seat !== 0) return;
				if (this.room.seats.length < BLASTER_MAX_PLAYERS) return;
				this.room.phase = "play";
				await this.save();
				this.broadcastRoom();
				this.hub.broadcastRaw(JSON.stringify({ t: "begin" } satisfies BlasterServerMsg));
				return;
			}
			case "input": {
			...
			case "over": {
				...
				await this.save();
				this.hub.broadcastRaw(JSON.stringify({ t: "ended", s: msg.s } satisfies BlasterServerMsg));
				this.broadcastRoom();
				return;
			}
		}
	...
	async webSocketClose(ws: WebSocket) {
		...
		if (seat !== -1) {
			...
			await this.save();
			this.hub.broadcastRaw(JSON.stringify({ t: "peerGone" } satisfies BlasterServerMsg), (a) => a.pid !== att.pid);
			this.broadcastRoom();
		}
	}
