...
// from /public/asteroids-game.html. Embedding it in an iframe keeps its own
// code untouched while this route supplies the site header and the shared
// arcade high-score board.
export default function Asteroids({}: Route.ComponentProps) {
	const { board, pendingScore, justRanked, finishRun, submit, dismiss } = useHighScores("asteroids");
	...
	useEffect(() => {
		function onMessage(event: MessageEvent) {
			...
			if (!data || data.type !== "arcade:gameover" || data.game !== "asteroids") return;
	...
	return (
		<div style={{ fontFamily: "'Inter', sans-serif", background: COLORS.bg, color: COLORS.text, minHeight: "100vh" }}>
			...
			>
				<h1 style={{ fontFamily: "'Archivo Black', sans-serif", fontSize: 32, margin: "0 0 8px" }}>Asteroids</h1>
