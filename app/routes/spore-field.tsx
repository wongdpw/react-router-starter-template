import type { Route } from "./+types/spore-field";
...
// from /public/spore-field-game.html. Embedding it in an iframe keeps its own
// code untouched while this route supplies the site header and the shared
// arcade high-score board.
export default function SporeField({}: Route.ComponentProps) {
	const { board, pendingScore, justRanked, finishRun, submit, dismiss } = useHighScores("spore-field");
	...
	useEffect(() => {
		function onMessage(event: MessageEvent) {
			...
			if (!data || data.type !== "arcade:gameover" || data.game !== "spore-field") return;
