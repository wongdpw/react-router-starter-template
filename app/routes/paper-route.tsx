...
export function meta({}: Route.MetaArgs) {
	return [{ title: "PaperBoy — Games — ArtDrop Spot" }];
}
...
// from /public/paper-route-game.html. Embedding it in an iframe keeps its own
...
export default function PaperRoute({}: Route.ComponentProps) {
	...
	useEffect(() => {
		function onMessage(event: MessageEvent) {
			...
			if (!data || data.type !== "arcade:gameover" || data.game !== "paper-route") return;
	...
	return (
		<div style={{ fontFamily: "'Inter', sans-serif", background: COLORS.bg, color: COLORS.text, minHeight: "100vh" }}>
			...
			>
				<h1 style={{ fontFamily: "'Archivo Black', sans-serif", fontSize: 32, margin: "0 0 8px" }}>PaperBoy</h1>
